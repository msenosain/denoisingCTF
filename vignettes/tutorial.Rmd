...
knitr::opts_knit$set(root.dir = "../../../../test_CyTOF/test2")
```
...
_should_ be expressed in the cells (e.g. post-fixation DNA intercalator). Any 
cell/event/row that has zero expression of this marker is removed. In this example
our "mandatory" marker is Histone H3: `##[52,] "Yb176Di_176Yb_His-H3"`.
```{r, echo=FALSE}
...
From the __n__ 'good' files, 0.75 will be randomly pulled for the training set
...
algorithm to use in the argument `alg` (see [documentation]([tutorial](https://msenosain.github.io/denoisingCTF/reference/TrainModel.html)) for details). 
```{r, eval = FALSE}
